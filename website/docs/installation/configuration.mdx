...
-   `AUTHENTIK_POSTGRESQL__CONN_MAX_AGE`: Set the PostgreSQL connection max age. Defaults to unlimited persistent connections. If using PgBouncer to handle persistent connections, this should be disabled with `0`.
...
All PostgreSQL settings, apart from `USE_PGBOUNCER`, `USE_PGPOOL`, and `CONN_MAX_AGE`, support hot-reloading. Adding and removing read replicas doesn't support hot-reloading.
